...
To move this data into a database that is persistent between restarts and across multiple IdentityServer instances, we can use the IdentityServer9 Entity Framework library.
...
IdentityServer9.EntityFramework
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
``IdentityServer9.EntityFramework`` implements the required stores and services using the following DbContexts:
...
You can find these contexts, their entities, and the IdentityServer9 stores that use them in the ``IdentityServer9.EntityFramework.Storage`` nuget package.

You can find the extension methods to register them in your IdentityServer in ``IdentityServer9.EntityFramework``, which we will do now::

    dotnet add package IdentityServer9.EntityFramework
...
The ``IdentityServer9.EntityFramework.Storage`` package contains entity classes that map from IdentityServer’s models.
As IdentityServer’s models change, so will the entity classes in ``IdentityServer9.EntityFramework.Storage``.
As you use ``IdentityServer9.EntityFramework.Storage`` and upgrade over time, you are responsible for your database schema and changes necessary to that schema as the entity classes change.
...
.. Note:: You can find the `latest SQL scripts <https://github.com/alexhiggins732/IdentityServer9/tree/main/src/EntityFramework.Storage/migrations/SqlServer/Migrations>`_ for SqlServer in the IdentityServer9.EntityFramework.Storage repository.
...
These methods each require a ``DbContextOptionsBuilder``, meaning your code will look something like this::
    ...
    const string connectionString = @"Data Source=(LocalDb)\MSSQLLocalDB;database=IdentityServer9.Quickstart.EntityFramework-4.0.0;trusted_connection=yes;";
